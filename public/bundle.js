...
var _life = __webpack_require__(301);

var _life2 = _interopRequireDefault(_life);

var _immutable = __webpack_require__(113);
...
var state = (0, _immutable.fromJS)([{ x: 1, y: 0 }, { x: 3, y: 1 }, { x: 0, y: 2 }, { x: 1, y: 2 }, { x: 4, y: 2 }, { x: 5, y: 2 }, { x: 6, y: 2 }]).toSet();
...
var worldToView = function worldToView(x, y) {
...
var draw = function draw() {
  ...
  state.forEach(function (cell) {
    var x = cell.get('x');
    var y = cell.get('y');
    context.fillRect.apply(context, _toConsumableArray(worldToView(x, y)).concat([scale, scale]));
  });
...
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(114), __webpack_require__(300)(module), __webpack_require__(298)))
...
var N = (0, _immutable.Range)(-1, 2).map(function (x, i, r) {
  return r.map(function (y) {
    return (0, _immutable.Map)({ x: x, y: y });
  });
}).flatten(true).toSet().delete((0, _immutable.Map)({ x: 0, y: 0 }));

var join = function join(a, b) {
  return (0, _immutable.Map)({ x: a.get('x') + b.get('x'), y: a.get('y') + b.get('y') });
};

var life = function life(state) {
  var neighbors = state.reduce(function (cells, cell) {
    return cells.union(N.map(function (n) {
      return join(cell, n);
    }));
  }, new _immutable.Set());
  var nextState = neighbors.union(state).reduce(function (cells, cell) {
    var count = N.reduce(function (c, n) {
      return state.has(join(cell, n)) ? c + 1 : c;
    }, 0);
